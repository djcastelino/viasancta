export const holyFootprintsChallenges = [
  {
    id: "holy-footprints-001",
    title: "Holy Footprints #001",
    subtitle: "Mystery Trail",
    category: "Apostle",
    displayAnswer: "St. Paul",
    acceptedAnswers: ["st paul", "saint paul", "paul", "apostle paul", "st. paul"],
    finalExplanation:
      "This journey follows Paul from his birthplace in Tarsus, to his conversion near Damascus, his missionary base in Antioch, his preaching in Athens, and his final witness in Rome.",
    funFacts: [
      "Paul was born in Tarsus and was originally known as Saul.",
      "His conversion is closely associated with the road to Damascus.",
      "Antioch became a major missionary center for the early Church.",
      "In Athens, Paul preached about the unknown God at the Areopagus.",
      "Tradition holds that Paul was martyred in Rome."
    ],
    stops: [
      {
        name: "Tarsus",
        x: 58,
        y: 42,
        locationClue: "A city in Cilicia, in modern-day Turkey.",
        faithClue: "This person was born here and originally known as Saul."
      },
      {
        name: "Damascus",
        x: 59,
        y: 44,
        locationClue: "One of the world's oldest continuously inhabited cities.",
        faithClue: "On the road to this city, this person had a life-changing encounter with Christ."
      },
      {
        name: "Antioch",
        x: 59,
        y: 41,
        locationClue: "A major city of the Roman Empire near the eastern Mediterranean.",
        faithClue: "This city became a major center of early Christian missionary activity."
      },
      {
        name: "Athens",
        x: 54,
        y: 46,
        locationClue: "A Greek city famous for philosophy and public debate.",
        faithClue: "This person preached here about the unknown God."
      },
      {
        name: "Rome",
        x: 52,
        y: 44,
        locationClue: "The capital of the Roman Empire.",
        faithClue: "Tradition says this person was martyred here."
      }
    ]
  },
  {
    id: "holy-footprints-002",
    title: "Holy Footprints #002",
    subtitle: "Mystery Trail",
    category: "Apostle",
    displayAnswer: "St. Peter",
    acceptedAnswers: ["st peter", "saint peter", "peter", "simon peter", "st. peter"],
    finalExplanation:
      "This journey follows Peter from his Galilean roots, through his leadership in the early Church, to his final witness in Rome.",
    funFacts: [
      "Peter was originally called Simon.",
      "Jesus renamed him Peter, meaning rock.",
      "Peter was a fisherman before becoming an apostle.",
      "He became a leading voice in the early Church.",
      "Tradition holds that Peter was martyred in Rome."
    ],
    stops: [
      {
        name: "Bethsaida",
        x: 59,
        y: 45,
        locationClue: "A fishing village near the Sea of Galilee.",
        faithClue: "This person was born near here and began life as a fisherman."
      },
      {
        name: "Capernaum",
        x: 59,
        y: 45,
        locationClue: "A town on the northern shore of the Sea of Galilee.",
        faithClue: "This person followed Jesus closely during his Galilean ministry."
      },
      {
        name: "Jerusalem",
        x: 59,
        y: 46,
        locationClue: "The holy city where the early Church began preaching after Pentecost.",
        faithClue: "This apostle became a leading voice here."
      },
      {
        name: "Antioch",
        x: 59,
        y: 41,
        locationClue: "A major early Christian city near the eastern Mediterranean.",
        faithClue: "Tradition connects this person with leadership in this city before Rome."
      },
      {
        name: "Rome",
        x: 52,
        y: 44,
        locationClue: "The capital of the Roman Empire.",
        faithClue: "Tradition says this apostle gave his final witness here."
      }
    ]
  },
  {
    id: "holy-footprints-003",
    title: "Holy Footprints #003",
    subtitle: "Mystery Trail",
    category: "Saint",
    displayAnswer: "St. Francis of Assisi",
    acceptedAnswers: [
      "st francis",
      "saint francis",
      "francis of assisi",
      "st francis of assisi",
      "saint francis of assisi",
      "st. francis of assisi"
    ],
    finalExplanation:
      "This journey follows Francis from Assisi, through his conversion and mission of poverty, to the places associated with his love for Christ, creation, and the Gospel.",
    funFacts: [
      "Francis was born in Assisi, Italy.",
      "He embraced radical poverty after his conversion.",
      "San Damiano is associated with the call to rebuild the Church.",
      "La Verna is associated with the stigmata.",
      "Greccio is connected with the first Christmas crèche tradition."
    ],
    stops: [
      {
        name: "Assisi",
        x: 52,
        y: 43,
        locationClue: "A hill town in Umbria, Italy.",
        faithClue: "This saint was born here and became known for poverty, peace, and joy."
      },
      {
        name: "San Damiano",
        x: 52,
        y: 43,
        locationClue: "A small church near Assisi.",
        faithClue: "Here, this saint heard Christ call him to rebuild the Church."
      },
      {
        name: "Rome",
        x: 52,
        y: 44,
        locationClue: "The center of the Catholic Church.",
        faithClue: "This saint sought approval for his new way of Gospel life here."
      },
      {
        name: "La Verna",
        x: 53,
        y: 43,
        locationClue: "A mountain sanctuary in Tuscany.",
        faithClue: "This place is associated with this saint receiving the stigmata."
      },
      {
        name: "Greccio",
        x: 52,
        y: 44,
        locationClue: "A small Italian town in the Rieti valley.",
        faithClue: "This saint helped create a living Christmas scene here."
      }
    ]
  },
  {
    id: "holy-footprints-004",
    title: "Holy Footprints #004",
    subtitle: "Mystery Trail",
    category: "Founder",
    displayAnswer: "St. Ignatius of Loyola",
    acceptedAnswers: [
      "st ignatius",
      "saint ignatius",
      "ignatius of loyola",
      "st ignatius of loyola",
      "saint ignatius of loyola",
      "st. ignatius of loyola"
    ],
    finalExplanation:
      "This journey follows Ignatius from his noble beginnings and injury at Loyola, through his conversion at Montserrat and Manresa, to his studies in Paris and mission in Rome.",
    funFacts: [
      "Ignatius was wounded in battle before his conversion deepened.",
      "Montserrat and Manresa were key places in his spiritual transformation.",
      "His Spiritual Exercises became one of the most influential works of Catholic spirituality.",
      "He studied in Paris.",
      "He founded the Society of Jesus, the Jesuits."
    ],
    stops: [
      {
        name: "Loyola",
        x: 49,
        y: 43,
        locationClue: "A place in the Basque region of Spain.",
        faithClue: "This saint's conversion began after a serious injury here."
      },
      {
        name: "Montserrat",
        x: 50,
        y: 44,
        locationClue: "A mountain monastery near Barcelona.",
        faithClue: "This saint made a decisive act of devotion here."
      },
      {
        name: "Manresa",
        x: 50,
        y: 44,
        locationClue: "A town in Catalonia, Spain.",
        faithClue: "This place deeply shaped this saint's spiritual exercises."
      },
      {
        name: "Paris",
        x: 50,
        y: 37,
        locationClue: "A major European center of learning.",
        faithClue: "This saint studied here and gathered companions."
      },
      {
        name: "Rome",
        x: 52,
        y: 44,
        locationClue: "The city at the heart of Catholic leadership.",
        faithClue: "This saint founded a religious order that would serve the Church worldwide."
      }
    ]
  },
  {
    id: "holy-footprints-005",
    title: "Holy Footprints #005",
    subtitle: "Mystery Trail",
    category: "Church Father",
    displayAnswer: "St. Augustine",
    acceptedAnswers: [
      "st augustine",
      "saint augustine",
      "augustine",
      "augustine of hippo",
      "st augustine of hippo",
      "st. augustine"
    ],
    finalExplanation:
      "This journey follows Augustine from North Africa through his restless search for truth, his conversion in Milan, and his ministry as bishop of Hippo.",
    funFacts: [
      "Augustine was born in North Africa.",
      "His mother, St. Monica, prayed for his conversion.",
      "He was baptized by St. Ambrose in Milan.",
      "His Confessions is one of the most famous spiritual autobiographies.",
      "He became bishop of Hippo."
    ],
    stops: [
      {
        name: "Thagaste",
        x: 51,
        y: 47,
        locationClue: "A town in Roman North Africa.",
        faithClue: "This saint was born here to a mother remembered for persistent prayer."
      },
      {
        name: "Carthage",
        x: 51,
        y: 46,
        locationClue: "A major ancient city in North Africa.",
        faithClue: "This person studied and struggled with ambition and desire here."
      },
      {
        name: "Rome",
        x: 52,
        y: 44,
        locationClue: "The capital of the Roman Empire.",
        faithClue: "This person spent time here before moving toward a decisive conversion."
      },
      {
        name: "Milan",
        x: 51,
        y: 42,
        locationClue: "A northern Italian city associated with St. Ambrose.",
        faithClue: "This person was baptized here after a long search for truth."
      },
      {
        name: "Hippo",
        x: 51,
        y: 47,
        locationClue: "A North African city where this saint became bishop.",
        faithClue: "This saint served the Church here as a bishop and theologian."
      }
    ]
  },
  {
    id: "holy-footprints-006",
    title: "Holy Footprints #006",
    subtitle: "Mystery Trail",
    category: "Modern Saint",
    displayAnswer: "St. Teresa of Calcutta",
    acceptedAnswers: [
      "st teresa of calcutta",
      "saint teresa of calcutta",
      "mother teresa",
      "st mother teresa",
      "saint mother teresa",
      "teresa of calcutta"
    ],
    finalExplanation:
      "This journey follows Teresa from Skopje to Ireland, India, and Kolkata, where she became known worldwide for serving Christ in the poorest of the poor.",
    funFacts: [
      "She was born in Skopje.",
      "She joined the Sisters of Loreto and traveled to India.",
      "She served many years in Kolkata.",
      "She founded the Missionaries of Charity.",
      "She is remembered for seeing Jesus in the poor and abandoned."
    ],
    stops: [
      {
        name: "Skopje",
        x: 54,
        y: 43,
        locationClue: "A city in the Balkans.",
        faithClue: "This modern saint was born here before becoming known across the world."
      },
      {
        name: "Dublin",
        x: 48,
        y: 32,
        locationClue: "The capital of Ireland.",
        faithClue: "This person came here while joining a religious community connected to missionary work."
      },
      {
        name: "Darjeeling",
        x: 76,
        y: 48,
        locationClue: "A hill town in India near the Himalayas.",
        faithClue: "This person began religious formation in India near this region."
      },
      {
        name: "Kolkata",
        x: 77,
        y: 50,
        locationClue: "A major city in eastern India.",
        faithClue: "This saint became famous for serving the poorest of the poor here."
      },
      {
        name: "Rome",
        x: 52,
        y: 44,
        locationClue: "The city where many saints are formally canonized.",
        faithClue: "This person was declared a saint by the Church here."
      }
    ]
  },
  {
    id: "holy-footprints-007",
    title: "Holy Footprints #007",
    subtitle: "Mystery Trail",
    category: "Doctor of the Church",
    displayAnswer: "St. Thomas Aquinas",
    acceptedAnswers: [
      "st thomas aquinas",
      "saint thomas aquinas",
      "thomas aquinas",
      "aquinas",
      "st. thomas aquinas"
    ],
    finalExplanation:
      "This journey follows Thomas Aquinas from his Italian roots and early education, through Naples and Paris, to his final days at Fossanova.",
    funFacts: [
      "Thomas Aquinas is one of the greatest theologians in Catholic history.",
      "He was a Dominican friar.",
      "His Summa Theologiae became a major work of Catholic theology.",
      "He studied and taught in Paris.",
      "He is called the Angelic Doctor."
    ],
    stops: [
      {
        name: "Roccasecca",
        x: 53,
        y: 45,
        locationClue: "A town in central Italy.",
        faithClue: "This great theologian was born near here."
      },
      {
        name: "Monte Cassino",
        x: 53,
        y: 45,
        locationClue: "A famous Benedictine monastery in Italy.",
        faithClue: "This person received early education connected with this monastery."
      },
      {
        name: "Naples",
        x: 53,
        y: 46,
        locationClue: "A major city in southern Italy.",
        faithClue: "This person studied here and encountered the Dominican order."
      },
      {
        name: "Paris",
        x: 50,
        y: 37,
        locationClue: "A great medieval university city.",
        faithClue: "This person studied and taught theology here."
      },
      {
        name: "Fossanova",
        x: 53,
        y: 45,
        locationClue: "An abbey in Italy.",
        faithClue: "This saint died here while traveling to a Church council."
      }
    ]
  },
  {
    id: "holy-footprints-008",
    title: "Holy Footprints #008",
    subtitle: "Mystery Trail",
    category: "Marian Apparition",
    displayAnswer: "St. Bernadette",
    acceptedAnswers: [
      "st bernadette",
      "saint bernadette",
      "bernadette",
      "bernadette soubirous",
      "st bernadette soubirous",
      "st. bernadette"
    ],
    finalExplanation:
      "This journey follows Bernadette from Lourdes, where she witnessed the apparitions of Our Lady, to Nevers, where she lived religious life and where her body is venerated.",
    funFacts: [
      "Bernadette was from Lourdes, France.",
      "She reported apparitions of the Blessed Virgin Mary in 1858.",
      "The grotto at Massabielle became one of the world's most famous pilgrimage sites.",
      "She later entered religious life in Nevers.",
      "Her life was marked by humility and suffering."
    ],
    stops: [
      {
        name: "Lourdes",
        x: 49,
        y: 43,
        locationClue: "A town in southwestern France near the Pyrenees.",
        faithClue: "This young saint lived here when Marian apparitions occurred."
      },
      {
        name: "Massabielle",
        x: 49,
        y: 43,
        locationClue: "A grotto near Lourdes.",
        faithClue: "This place became central to the apparitions this saint witnessed."
      },
      {
        name: "Nevers",
        x: 50,
        y: 40,
        locationClue: "A city in central France.",
        faithClue: "This saint later lived religious life here."
      }
    ]
  },
  {
    id: "holy-footprints-009",
    title: "Holy Footprints #009",
    subtitle: "Mystery Trail",
    category: "Marian Apparition",
    displayAnswer: "St. Juan Diego",
    acceptedAnswers: [
      "st juan diego",
      "saint juan diego",
      "juan diego",
      "juan diego cuauhtlatoatzin",
      "st. juan diego"
    ],
    finalExplanation:
      "This journey follows Juan Diego from his home region to Tepeyac Hill, where Our Lady of Guadalupe appeared, and to Mexico City, where the miraculous image became a lasting sign of faith.",
    funFacts: [
      "Juan Diego is associated with Our Lady of Guadalupe.",
      "The apparitions occurred in 1531.",
      "Tepeyac Hill became one of the most important pilgrimage sites in the Americas.",
      "The image of Our Lady appeared on his tilma.",
      "He was canonized in 2002."
    ],
    stops: [
      {
        name: "Cuautitlán",
        x: 20,
        y: 50,
        locationClue: "A place in central Mexico.",
        faithClue: "This saint is traditionally associated with this region."
      },
      {
        name: "Tepeyac Hill",
        x: 20,
        y: 50,
        locationClue: "A hill near Mexico City.",
        faithClue: "This person encountered Our Lady here."
      },
      {
        name: "Mexico City",
        x: 20,
        y: 50,
        locationClue: "The capital of Mexico.",
        faithClue: "The miraculous image connected to this saint became venerated here."
      }
    ]
  },
  {
    id: "holy-footprints-010",
    title: "Holy Footprints #010",
    subtitle: "Mystery Trail",
    category: "Modern Pope",
    displayAnswer: "St. John Paul II",
    acceptedAnswers: [
      "st john paul ii",
      "saint john paul ii",
      "john paul ii",
      "pope john paul ii",
      "karol wojtyla",
      "st. john paul ii"
    ],
    finalExplanation:
      "This journey follows John Paul II from Wadowice and Kraków to Rome and Vatican City, with Fatima marking a powerful Marian connection in his life and papacy.",
    funFacts: [
      "John Paul II was born Karol Wojtyła in Wadowice, Poland.",
      "He served as archbishop of Kraków before becoming pope.",
      "He became pope in 1978.",
      "He had a deep devotion to Our Lady of Fatima.",
      "He traveled widely and became one of the most recognized popes in modern history."
    ],
    stops: [
      {
        name: "Wadowice",
        x: 53,
        y: 38,
        locationClue: "A town in southern Poland.",
        faithClue: "This modern saint and future pope was born here."
      },
      {
        name: "Kraków",
        x: 54,
        y: 38,
        locationClue: "A historic Polish city.",
        faithClue: "This person served the Church here before becoming pope."
      },
      {
        name: "Rome",
        x: 52,
        y: 44,
        locationClue: "The city associated with the papacy.",
        faithClue: "This person was elected pope here."
      },
      {
        name: "Fatima",
        x: 47,
        y: 45,
        locationClue: "A Marian pilgrimage site in Portugal.",
        faithClue: "This pope had a deep connection to this apparition site."
      },
      {
        name: "Vatican City",
        x: 52,
        y: 44,
        locationClue: "The small sovereign city-state at the heart of the Catholic Church.",
        faithClue: "This saint led the universal Church from here."
      }
    ]
  }
];
